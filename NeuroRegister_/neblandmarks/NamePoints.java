...
class PointsDialog extends Dialog implements ActionListener, WindowListener {
	...
	public PointsDialog(String title,
		...
		rc.gridy = 0;
		// nebula
		registrationPanel.add( new Label("Register to template (print Transform Matrix) : "), rc );
		registerRigid = new Button( "Best Rigid" );
		registerAffine = new Button( "Affine Registration" );
		++ rc.gridx;
		...
		rc.gridx = 0;
		rc.gridy = 1;
		registrationPanel.add( new Label("Register to template with SWC : "), rc );
		//registerBookstein = new Button( "Thin-Plate Spline Registration" );
		registerBookstein2 = new Button( "Thin Plate Spline" );
		//++ rc.gridx;
		//registrationPanel.add( registerBookstein, rc );
		//registerBookstein.addActionListener( this );
		++ rc.gridx;
		registrationPanel.add( registerBookstein2, rc );
		registerBookstein2.addActionListener( this );

		withInvert = new Checkbox("with Invert");
		rc.gridx = 0;
		rc.gridy = 2;
		registrationPanel.add( withInvert, rc );
...
*/
		...
		} else if( source == registerBookstein2 ) {
			plugin.doRegistration(NamePoints.BOOKSTEIN_SWC);
		}
	}
...
public class NamePoints implements FineTuneProgressListener {
	...
	public void doRegistration( int method ) {
		...
		case AFFINE:
			...
			transformed = ((AffineFromLandmarks)r).register( templatePoints, points );
			break;
		case RIGID:
			r = new RigidFromLandmarks();
			r.setImages( templateImage, imp );
			transformed = ((RigidFromLandmarks)r).register( templatePoints, points );
			break;
		case BOOKSTEIN:
			r = new BooksteinFromLandmarks();
			r.setImages( templateImage, imp );
			transformed = ((BooksteinFromLandmarks)r).register( templatePoints, points );
			break;
		case BOOKSTEIN_SWC:
			r = new BooksteinFromLandmarks();
			r.setImages( templateImage, imp );
			transformed = ((BooksteinFromLandmarks)r).register( templatePoints, points, "~/tmp.swc" , withInvert, overlayResult);
			break;
		default:
		...
		if( transformed == null ){
			IJ.error("Transform Error!");
			return;
		}


		if( overlayResult == true) {
			if(method!=BOOKSTEIN_SWC){
